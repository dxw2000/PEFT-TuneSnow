...
class Baseline_Finetune_MSPM_DARM(nn.Module):
    def __init__(self,
    in_channels=3,
    out_cahnnels=3,
    ...
    checkpoint="/mnt/Temp_new/dxw/code/SAM_desnow/exp_SAM_desnow/SAM_ckpt/sam_vit_b_01ec64.pth",   # "sam_vit_b_01ec64.pth" "sam_vit_l_0b3195.pth" "sam_vit_h_4b8939.pth"
    SAM_image_size = 256,
    ):
        super(Baseline_Finetune_MSPM_DARM, self).__init__()
        self.patch_size = patch_size
    ...
    def forward(self,x):
        ...
        SAM_enc_level0 = F.upsample(x_0_1, (inp_enc_level0.shape[2], inp_enc_level0.shape[3]), mode="bicubic")
        ...
        SAM_enc_level1 = F.upsample(x_2_3_4, (inp_enc_level1.shape[2], inp_enc_level1.shape[3]), mode="bicubic")
        ...
        SAM_enc_level2 = F.upsample(x_5_6_7, (inp_enc_level2.shape[2], inp_enc_level2.shape[3]), mode="bicubic")
        ...
        SAM_enc_level3 = F.upsample(x_8_9_10_11, (inp_enc_level3.shape[2], inp_enc_level3.shape[3]), mode="bicubic")
...
# model = Baseline_Finetune_MSPM_DARM().cuda()
# input_img = torch.rand((1, 3, 128, 128)).cuda()
# output_img = model(input_img)
# print("output_img", output_img.shape)


# def cal_time(model_t, cal_iter=10):
#
#     input_test_tensor = torch.randn(size=(1, 3, 128, 128))
#
...
#     with torch.no_grad():
#
#         for i in range(cal_iter):
#
#             start.record()
#
#             out = model_t(input_test_tensor)
#
#             end.record()
#
...
# # 模型定义
# model = Baseline_Finetune_MSPM_DARM().to(device)
#
# # 获取模型占用的显存
# allocated_memory = torch.cuda.memory_allocated(device) / (1024**2)  # 转换为兆字节
# cached_memory = torch.cuda.memory_cached(device) / (1024**2)  # 转换为兆字节
#
